...
_isr8:
  cli
  push byte 8        ; Error code is already pushed onto the stack
  jmp isr_common_stub
...
; isr_common_stub uses a C function to handle interrupts
extern fault_handler

; Save process state, sets up for kernel mode segments, calls C-level fault
; handler, and finally restores stack frame.
isr_common_stub:
  pusha
  push ds
  push es
  ...
  call eax        ; A special call, preserves the 'eip' register ?? what mean ?
  pop eax
  pop gs
  pop fs
  pop es
